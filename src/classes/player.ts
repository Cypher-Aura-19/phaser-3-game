import Phaser from 'phaser';
import SCENE from './scene';
import { GAME_CONFIG } from '../config';

export default class PLAYER extends Phaser.Physics.Arcade.Sprite {
    declare scene: SCENE;
    cursors: Phaser.Types.Input.Keyboard.CursorKeys;
    wasd: Record<string, Phaser.Input.Keyboard.Key>;
    touchY: number;
    click: number;

    constructor(scene: SCENE, x: number, y: number, texture: string, frame: string | number) {
        super(scene, x, y, texture, frame);
        
        this.scene.add.existing(this);
        this.scene.physics.add.existing(this);
        this.setInteractive();
        this.setBounce(0.25);
        this.setDepth(25); // z-index
        this.setTint(scene.theme.player);
        this.createAnims();

        // Input Events
        if (scene.input.keyboard) {
            this.cursors = scene.input.keyboard.createCursorKeys();
            this.wasd = scene.input.keyboard.addKeys('W,S,A,D,SPACE') as Record<string, Phaser.Input.Keyboard.Key>;
        } else {
            // Fallback for types
            this.cursors = {} as any;
            this.wasd = {} as any;
        }
        scene.input.addPointer(1); // for multi-touch
        this.touchY = 0;
        this.click = 0; // duration of last click
    }

    createAnims() {
        // Only create animations if they don't already exist in the global animation manager
        if (this.scene.anims.exists('turn')) return;

        this.scene.anims.create({
            key: 'turn',
            frames: [ { key: 'dude', frame: 0 } ],
            frameRate: 20
        });

        this.scene.anims.create({
            key: 'left',
            frames: this.scene.anims.generateFrameNumbers('dude', { start: 1, end: 3 }),
            frameRate: 8,
            repeat: -1
        });
    
        this.scene.anims.create({
            key: 'right',
            frames: this.scene.anims.generateFrameNumbers('dude', { start: 6, end: 8 }),
            frameRate: 8,
            repeat: -1
        });

        this.scene.anims.create({
            key: 'dance',
            frames: this.scene.anims.generateFrameNumbers('dude', { start: 0, end: 12 }),
            frameRate: 8,
            repeat: -1
        });        

        this.scene.anims.create({
            key: 'jump',
            frames: [ { key: 'dude', frame: 11 } ],
            frameRate: 20,
            repeat: -1
        });      

        this.scene.anims.create({
            key: 'jumpRight',
            frames: [ { key: 'dude', frame: 5 } ],
            frameRate: 20,
            repeat: -1
        });      

        this.scene.anims.create({
            key: 'jumpLeft',
            frames: [ { key: 'dude', frame: 4 } ],
            frameRate: 20,
            repeat: -1
        });

        this.scene.anims.create({
            key: 'stomp',
            frames: [ { key: 'dude', frame: 12 } ],
            frameRate: 20,
            repeat: -1
        });
    }

    playAnims(dir: string | boolean, jump: boolean, stomp: boolean, touch: boolean) {
        if (!this.scene.invulnerable) {
            if (jump) {
                this.setTint(this.scene.theme.playerInvulnerable);
            } else {
                this.setTint(this.scene.theme.player);
            }
        }

        if(dir === 'left') {
            this.setVelocityX(-GAME_CONFIG.playerSpeed);
            if(jump) {
                this.anims.play('jumpLeft', true);
            } else {
                this.anims.play('left', true);
            }
        } else if(dir === 'right') {
            this.setVelocityX(GAME_CONFIG.playerSpeed);
            if(jump) {
                this.anims.play('jumpRight', true);
            } else {
                this.anims.play('right', true);
            }
        } else {
            this.setVelocityX(0);
            if(jump) {                
                this.anims.play('jump');
            } else if(stomp) {
                this.anims.play('stomp');
                this.setVelocityY(GAME_CONFIG.playerStompVelocity);
            } else {
                this.anims.play('turn');
            }
        }        
    
        if(jump && this.body && this.body.touching.down){
            let isKeyboardJump = false;
            if (this.cursors.up && this.wasd.W && this.wasd.SPACE) {
                isKeyboardJump = Phaser.Input.Keyboard.JustDown(this.cursors.up) 
                    || Phaser.Input.Keyboard.JustDown(this.wasd.W)
                    || Phaser.Input.Keyboard.JustDown(this.wasd.SPACE);
            }
            if(touch || isKeyboardJump) {
                this.setVelocityY(GAME_CONFIG.playerJumpVelocity);
            }
        }
    }

    // based on user input
    move() {
        let touch = false; // track whether input is keyboard or touch

        let dirPointer = this.scene.input.pointer1;
        let jumpPointer = this.scene.input.pointer2;
    
        let jump = touch = jumpPointer.isDown ? true : false;
        let stomp = false;
        let dir: string | boolean = false;
    
        // touch controls
        if(dirPointer.isDown) {
            let x = dirPointer.position.x;
            let y = dirPointer.position.y;
            if(this.touchY === 0) {
                this.touchY = y;
            } else if(y > this.touchY + 50) {                
                stomp = true;
            } else {
                // midpoint responsive calculation instead of hardcoded 400
                const midpoint = this.scene.scale.width / 2;
                if(x > midpoint) {
                    dir = 'right';
                } else {            
                    dir = 'left';
                }
            }
        } else {            
            this.touchY = 0;
            dir = false;
            let dur = dirPointer.getDuration();
            if((this.click !== dur) && (dur > 0) && (dur < 150)) {
                // if dirPointer isn't down and screen is tapped, jump
                jump = touch = true;
                this.click = dur; // to disable autojumping
            }
        }

        // keyboard controls
        if (this.cursors.left && this.wasd.A) {
            if(this.cursors.left.isDown || this.wasd.A.isDown) {
                dir = 'left';
            } else if(this.cursors.right.isDown || this.wasd.D.isDown) {
                dir = 'right';
            }

            if(this.cursors.up.isDown || this.wasd.W.isDown || this.wasd.SPACE.isDown) {
                jump = true;
                touch = false;
            } else if(this.cursors.down.isDown || this.wasd.S.isDown) {
                stomp = true;
            }
        }
    
        this.playAnims(dir, jump, stomp, touch);
    }
}
